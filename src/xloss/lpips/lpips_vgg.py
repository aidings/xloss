...
class LPIPS(nn.Module):
    ...
    def load_from_pretrained(self, ckpt_path=None):
        ckpt_path = data_root / "vgg.pth" if ckpt_path is None else ckpt_path
        self.load_state_dict(torch.load(ckpt_path, map_location=torch.device("cpu")), strict=False)
        logger.info("loaded pretrained LPIPS loss from {}".format(ckpt_path))

    @classmethod
    def from_pretrained(cls, ckpt_path="vgg_lpips"):
        model = cls()
        ckpt_path = data_root / "vgg.pth" if ckpt_path is None else ckpt_path
