...
from ..utils import AdversarialCriterion, LossReduction
...
class PatchAdversarialLoss(_Loss):
    ...
    def forward(
        ...
        for _, disc_out in enumerate(input):
            if self.criterion != AdversarialCriterion.HINGE:
        ...
        for disc_ind, disc_out in enumerate(input):
            ...
            if self.criterion == AdversarialCriterion.HINGE and not target_is_real:
    ...
    def _forward_single(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if self.criterion == AdversarialCriterion.BCE or self.criterion == AdversarialCriterion.LEAST_SQUARE:
            forward = self.loss_fct(input, target)
        elif self.criterion == AdversarialCriterion.HINGE:
